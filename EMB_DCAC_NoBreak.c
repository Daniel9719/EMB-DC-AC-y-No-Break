...
void Inic_puertos(void){
    ...
    NVIC_PRI12_R=(NVIC_PRI12_R&0x0FFFFFFF)|0xC0000000;//Prioridad 6 (Menor)
...
void Inicial_Timer(void){
    SYSCTL_RCGCTIMER_R|=0x60;                       //Reloj del Timer 6 y 5
    ...
    NVIC_PRI16_R=(NVIC_PRI16_R&0xFFFF0FFF)|0x0000E000;//Prioridad 7 (Menor)
...
//    TIMER5_TAMATCH_R
    ...
    NVIC_PRI16_R=(NVIC_PRI16_R&0xFFFF0FFF)|0x0000E000;//Prioridad 7 (Menor)
    NVIC_EN2_R|=((1<<(65-64)) & 0xFFFFFFFF);        //Hab interrupción del Timer 4
//    TIMER4_TAMATCH_R
...
void Inicial_ADC0(void){
    ...
    NVIC_PRI4_R=(NVIC_PRI4_R&0xFFFF0FFF)|0x0000E000;//Prioridad 7 (Menor)
...
//    ADC0_DCCTL0|=0x14;                            //Comparador digital 0 genera una interrupción si se está en banda media
    ...
    ADC0_PSSI_R=0x8;                                //Inicio conversión Seq0
}
...
void Inter_ADC_Seq3(void){
    ...
    ADC0_ISC_R=0x80000;                             //Limpieza bandera RIS ADC0 Seq0
...
void Inter_PortJ_Falla(void){
    ...
    if(GPIO_PORTJ_AHB_DATA_R&0x1){                  //Termina la falla
        TIMER4_IM_R=0x0;                            //Deshab interrupción Match Timer A
        TIMER5_IM_R=0x0;                            //Deshab interrupción Match Timer A
        ADC0_IM_R|=0x80000;                         //Hab interrupción del secuenciador 3
    }else{                                          //Ocurre la falla
        ADC0_IM_R&=~0x80000;                        //Deshab interrupción del secuenciador 3
        if(){
            FETp_FETn=0x8;
        }else{TIMER6_TAV_R<(Periodo/2)
            FETp_FETn=0x4;
        }
        ...
        TIMER4_IM_R|=0x10;                          //Hab interrupción Match Timer A
        TIMER5_IM_R|=0x10;                          //Hab interrupción Match Timer A
    }
}
